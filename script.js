...
if (typeof SpeechRecognition === "undefined") {
  ...
  recognition.onresult = async event => {
    ...
    if (recognitionResult.isFinal) {
      ...
      const response = await sendMessage(recognitionText);
      const p = document.createElement("p");
      p.innerHTML = `<strong>You said:</strong> ${recognitionText} </br><strong>RAT-GPT said:</strong> ${response}`;
...
async function sendMessage(message) {
  ...
  customHeaders.append("Authorization",`Bearer ${OPENAI_KEY}`)
  let requestOptions = {
    method: 'POST',
